...
from scripts import evaluate_nested_catalog_descriptor_snapshot as snapshot


class NestedCatalogDescriptorSnapshotTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_locked_nested_snapshot_gate_passes(self):
        self.assertTrue(snapshot.accepted(self._summary(), self.fixture))

    def test_shared_and_shallow_drift_but_deep_does_not(self):
        summary = self._summary()
        self.assertEqual(summary["shared_correct"], 0)
        self.assertEqual(summary["shallow_correct"], 0)
        self.assertEqual(summary["deep_correct"], 3)
        self.assertEqual(summary["shared_drifts"], 3)
        self.assertEqual(summary["shallow_drifts"], 3)
        self.assertEqual(summary["deep_drifts"], 0)

    def test_nested_aliases_and_mutations_are_locked(self):
        summary = self._summary()
        self.assertTrue(summary["alias_relations_locked"])
