#!/usr/bin/env python3
"""Evaluate the expert-package manifest contract against a key-only baseline."""
...
from scripts.validate_expert_manifest import TOP_LEVEL_KEYS, validate_manifest  # noqa: E402


DEFAULT_FIXTURE = ROOT / "fixtures" / "expert_manifest_cases.json"
...
def validate_fixture(fixture: dict[str, Any]) -> None:
    ...
    if fixture["schema"] != "expert-manifest-cases-v1":
    ...
    for case in cases:
        ...
        if not isinstance(mutation, dict) or mutation.get("op") not in {"none", "set", "remove"}:
...
def baseline_accepts(manifest: Any) -> bool:
    return isinstance(manifest, dict) and TOP_LEVEL_KEYS.issubset(manifest)
        ...
        results.append(
            {
                ...
                "baseline": "valid" if baseline_accepts(manifest) else "invalid",
...
def main() -> int:
    ...
    summary["accepted"] = (
        ...
        and summary["baseline_false_accepts"] >= 9
