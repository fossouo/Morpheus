#!/usr/bin/env python3
"""Evaluate package-owned exclusions for quarantined wildcard-routed experts."""
...
DEFAULT_FIXTURE = ROOT / "fixtures" / "expert_package_owned_exclusion_cases.json"


class PackageOwnedExclusionRoutingKernel(WildcardScopeRoutingKernel):
    """Filter each include candidate by exclusions declared by the same package."""

    def __init__(self) -> None:
        super().__init__()
        self._exclusions_by_package: dict[str, tuple[str, ...]] = {}
    ...
    def answer(self, request: Any) -> str:
        if isinstance(request, dict) and request.get("operation") == "scope_recall":
            ...
            best_score = max(score for score, _, _ in eligible)
            winners = [candidate for candidate in eligible if candidate[0] == best_score]
            if len(winners) != 1:
    ...
    def compose_quarantined_experts(
        ...
        if self._knowledge_by_package or self._package_by_scope or self._exclusions_by_package:
        ...
        proposed_exclusions: dict[str, tuple[str, ...]] = {}
        for package in packages:
            ...
            exclusions = tuple(package["manifest"]["scope"]["exclude"])
            for pattern in exclusions:
                _pattern_segments(pattern)
            proposed_exclusions[package_id] = exclusions
        ...
        self._exclusions_by_package = proposed_exclusions

    def unload_experts(self) -> None:
        super().unload_experts()
        self._exclusions_by_package = {}
...
def validate_fixture(fixture: Any) -> None:
    ...
    if fixture["schema"] != "expert-package-owned-exclusion-cases-v1":
    ...
    if (
        policies.count("allow") != 2
        or policies.count("cross-package-exact") != 1
        or policies.count("cross-package-wildcard") != 1
        or policies.count("all-excluded-exact") != 1
        or policies.count("all-excluded-wildcard") != 1
    ):
        raise ValueError("target policies must lock two allows, two recoveries, and two denials")
...
def run_trial(fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, order in enumerate(fixture["compatible_orders"]):
        ...
        baseline = ExclusionScopeRoutingKernel()
        ...
        candidate = PackageOwnedExclusionRoutingKernel()
...
def summarize(fixture: dict[str, Any], trial: dict[str, Any]) -> dict[str, Any]:
    ...
    cross_indexes = [
        index for index, case in enumerate(targets)
        if case["policy"].startswith("cross-package-")
    ]
    ...
    return {
        ...
        "baseline_cross_package_over_denials": sum(
            trial["baseline_target"][index] == "route-error:scope-excluded"
        ...
        "candidate_cross_package_recoveries": sum(
...
def main() -> int:
    ...
    summary["accepted"] = (
        ...
        and summary["baseline_cross_package_over_denials"] == 2
        and summary["candidate_target_correct"] == summary["target_count"] == 6
        and summary["target_accuracy_gain"] == 0.333333
        and summary["candidate_cross_package_recoveries"] == 2
