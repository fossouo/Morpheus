#!/usr/bin/env python3
"""Evaluate deterministic whole-segment wildcard routing for quarantined experts."""
...
DEFAULT_FIXTURE = ROOT / "fixtures" / "expert_wildcard_scope_routing_cases.json"
...
class WildcardScopeRoutingKernel(HierarchicalScopeRoutingKernel):
    """A fixed kernel ranking matching scope patterns by depth then literal count."""
        ...
        if isinstance(request, dict) and request.get("operation") == "scope_recall":
            ...
            for pattern, package_id in self._package_by_scope.items():
                ...
                if all(
                    ...
                    candidates.append((score, pattern, package_id))
            if not candidates:
...
def _validate_reversed_orders(name: str, orders: Any, package_ids: list[str]) -> None:
    if (
        not isinstance(orders, list)
        or len(orders) != 2
        or any(not isinstance(order, list) or len(order) != 2 for order in orders)
        or orders[1] != list(reversed(orders[0]))
        or any(package_id not in package_ids for order in orders for package_id in order)
    ):
        raise ValueError(f"{name} must contain a package pair and its reversal")
...
def validate_fixture(fixture: Any) -> None:
    ...
    if fixture["schema"] != "expert-wildcard-scope-routing-cases-v1":
...
def run_trial(fixture: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, order in enumerate(fixture["compatible_orders"]):
        ...
        baseline = HierarchicalScopeRoutingKernel()
        ...
        candidate = WildcardScopeRoutingKernel()
...
def summarize(fixture: dict[str, Any], trial: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "equal_specificity_rejections": sum(
            response == fixture["tie_probe"]["expected"]
            for response in trial["tie_responses"]
        ),
...
def main() -> int:
    ...
    summary["accepted"] = (
        ...
        and summary["target_accuracy_gain"] == 0.666667
